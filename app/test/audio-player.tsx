    ...
    <div className="shadow-none bg-white border border-gray-200 rounded-lg overflow-hidden">
      ...
      <div className="flex items-center justify-between gap-2 p-1 w-full">
        <div className="flex-1">
          ...
          >
            <TabsList className="h-8 bg-transparent w-full justify-between">
              {[0.2, 0.5, 0.75, 1, 1.25, 1.5, 2.0].map((rate) => (
                <TabsTrigger
                  key={rate}
                  value={rate.toString()}
                  className="text-sm px-2 rounded-full data-[state=active]:bg-red-600 data-[state=active]:text-white"
                >
                  {rate}x
                </TabsTrigger>
              ))}
